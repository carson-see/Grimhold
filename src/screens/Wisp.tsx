...
export function WispScene() {
  ...
  useEffect(() => {
    playMusicBoxNote({ freq: 698.46, detuneCents: -16, durationMs: 1800, gain: 0.13 });
    advanceTimer.current = window.setTimeout(() => setScreen('scene-01'), reduce ? 1200 : 2600);
    return () => {
  ...
  return (
    <Frame>
      ...
      <motion.div
        ...
        transition={{ duration: reduce ? 0.8 : 2.2, times: reduce ? undefined : [0, 0.15, 0.7, 0.9, 1], ease: 'easeOut' }}
      >
      ...
      <motion.div
        ...
        transition={{ delay: reduce ? 0.4 : 1.4, duration: reduce ? 0.6 : 1 }}
      />
      ...
      <motion.p
        ...
        transition={{ delay: reduce ? 0.2 : 1.8, duration: reduce ? 0.8 : 0.8 }}
      >
