...
// Mira at the door looking down the corridor. If Bessie was recruited, a
// small cloth bag is placed against the door base. If not, the corridor
// is empty. Mira says "I know."  First line of dialogue in the game.
...
export function Scene03() {
  ...
  useEffect(() => {
    timer.current = window.setTimeout(() => setCanAdvance(true), reduce ? 900 : 4400);
    const noteTimer = window.setTimeout(
      () => playMusicBoxNote({ freq: 415.3, detuneCents: -24, durationMs: 2000, gain: 0.11 }),
      reduce ? 400 : 2200,
    );
    return () => {
  ...
  const advance = () => setScreen('larder-stub');
  ...
  return (
    <Frame>
      ...
      <div className="relative z-10 flex flex-col h-[100dvh]">
        {/* The door — a heavy slab with a narrow window cut high up.
            Drawn inline so we don't spawn another single-use asset file. */}
        <div className="relative flex-1 flex items-end justify-center pb-12">
          ...
          {/* The bag — slid up against the door base, only if recruited */}
          {bessieAllyActive && (
            <motion.div
              aria-hidden="true"
